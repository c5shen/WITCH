...
class ConfigWITCH(Command):
    ...
    def run(self):
        ...
        print('Copying a self-contained MAGUS installation to {}...'.format(
            tools_dir))
        copied_magus_dir = self.copy_tool_to_lib(
                'magus', os.path.join(os.getcwd(), "tools", "magus"))
        cparser.set('Basic', 'magus_path', copied_magus_dir + '/magus.py')
        ...
        if 'macos' not in platform_name.lower():
            ...
            for _section in set_sections:
                ...
                cparser.set(_section, 'mafftpath',
                        os.path.join(copied_magus_dir, 'tools', 'mafft', 'mafft'))
                # mclpath
                cparser.set(_section, 'mclpath',
                        os.path.join(copied_magus_dir, 'tools', 'mcl', 'bin', 'mcl'))
                # fasttreepath
                cparser.set(_section, 'fasttreepath',
                        os.path.join(copied_magus_dir, 'tools', 'fasttree', 'FastTreeMP'))
                # hmmer packages
                for hmmer_pkg in ['hmmsearch', 'hmmalign', 'hmmbuild']:
                    cparser.set(_section, '{}path'.format(hmmer_pkg),
                            os.path.join(copied_magus_dir, 'tools', 'hmmer', hmmer_pkg))
        else:
            ...
            for binary in binaries:
                ...
                for _section in set_sections:
                    if 'FastTreeMP' in path:
                        ...
                        cparser.set(_section, '{}path'.format(binary),
...
setuptools.setup(
        ...
        package_data={"witch_msa": ["tools"]},
        include_package_data=True,
        cmdclass={"": ConfigWITCH},
